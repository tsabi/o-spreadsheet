...
describe("datasource tests", function () {
  ...
  describe("create gauge chart with invalid section rule", () => {
    ...
    test("LowerInflectionPoint > UpperInflectionPoint", async () => {
      sectionRule.lowerInflectionPoint.value = "66";
      sectionRule.upperInflectionPoint.value = "33";
      const result = createGaugeChart(
        model,
        {
          dataRange: "A1",
      ...
      expect(result).toBeCancelledBecause(CommandResult.GaugeLowerBiggerThanUpper);
    });
  });
...
describe("Chart design configuration", () => {
  const defaultChart: GaugeChartUIDefinition = {
    dataRange: "A1",
    title: "My chart",
    type: "gauge",
    sectionRule: defaultSectionRule,
  ...
  describe("dataRange < rangeMin", () => {
    ...
    test("displaying value always correspond to dataRange value", () => {
      createGaugeChart(model, defaultChart, "1");
      const chart = model.getters.getGaugeChartRuntime("1")!;
      const displayingValue = chart.options!.valueLabel!.formatter!;
      expect(displayingValue()).toBe("-60");
    });
  });
  ...
  describe("dataRange > rangeMax", () => {
    ...
    test("displaying value always correspond to dataRange value", () => {
      ...
      const chart = model.getters.getGaugeChartRuntime("1")!;
      const displayingValue = chart.options!.valueLabel!.formatter!;
      expect(displayingValue()).toBe("160");
    });
  });

  test("displaying value respect dataRange format value", () => {
    model = new Model();
    ...
    const chart = model.getters.getGaugeChartRuntime("1")!;
    const displayingValue = chart.options!.valueLabel!.formatter!;
    expect(displayingValue()).toBe("42.00%");
  });
  ...
  test("data configuration include inflection point values followed by rangeMax value", () => {
    ...
    expect(chart.data!.datasets![0].data).toBe([22, 42, 62]);
    expect(chart.data!.datasets![0].backgroundColor).toBe([lowerColor, middleColor, upperColor]);
  });
  ...
  test("don't take into account inflection point if empty", () => {
    ...
    createGaugeChart(model, defaultChart, "1");
    let chart = model.getters.getGaugeChartRuntime("1")!;
    expect(chart.data!.datasets![0].data).toBe([66, 100]);
    expect(chart.data!.datasets![0].backgroundColor).toBe([middleColor, upperColor]);
    ...
    expect(chart.data!.datasets![0].data).toBe([33, 100]);
    expect(chart.data!.datasets![0].backgroundColor).toBe([lowerColor, upperColor]);
  });

  test("don't take into account inflection point if below rangeMin  ", () => {
    const model = new Model();
    defaultChart.sectionRule.rangeMax = "100";
    defaultChart.sectionRule.lowerInflectionPoint.value = "-100";
    defaultChart.sectionRule.upperInflectionPoint.value = "66";
    createGaugeChart(model, defaultChart, "1");
    let chart = model.getters.getGaugeChartRuntime("1")!;
    expect(chart.data!.datasets![0].data).toBe([66, 100]);
    ...
    defaultChart.sectionRule.upperInflectionPoint.value = "-50";
    createGaugeChart(model, defaultChart, "2");
    chart = model.getters.getGaugeChartRuntime("2")!;
    expect(chart.data!.datasets![0].data).toBe([100]);
    expect(chart.data!.datasets![0].backgroundColor).toBe([upperColor]);
  });
});
function setFormat(model: Model, oneDecimal: any) {
