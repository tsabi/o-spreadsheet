...
describe("Grid component", () => {
  ...
  describe("keybindings", () => {
    ...
    test("Filter icon change when filter is active", async () => {
      ...
      await nextTick();
      expect(fixture.querySelectorAll(".filter-icon")).toHaveLength(1);
      expect(fixture.querySelectorAll(".filter-icon-active")).toHaveLength(0);

      updateFilter(model, "A1", ["5"]);
      await nextTick();
      expect(model.getters.isFilterActive(model.getters.getActiveSheetId(), 0, 0)).toBeTruthy();
      expect(fixture.querySelectorAll(".filter-icon")).toHaveLength(0);
      expect(fixture.querySelectorAll(".filter-icon-active")).toHaveLength(1);
    });
