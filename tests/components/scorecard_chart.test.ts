...
import { createScorecardChart } from "../test_helpers/commands_helpers";
...
describe("Scorecard charts", () => {
  ...
  test("Increasing size of the chart scale up the font sizes", async () => {
    ...
    expect(getElementFontSize(getChartTitleElement())).toBeGreaterThan(titleFontSize);
  ...
  test("Decreasing size of the chart scale down the font sizes", async () => {
    ...
    expect(getElementFontSize(getChartTitleElement())).toBeLessThan(titleFontSize);
    expect(getElementFontSize(getChartKeyElement())).toBeLessThan(keyFontSize);
  });

  test("Font size scale down if we put a long title", async () => {
    createScorecardChart(
      model,
      { keyValue: "A1", baseline: "B2", title: "This is a title" },
      chartId
