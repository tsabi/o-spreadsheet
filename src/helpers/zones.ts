...
export function toZoneWithoutBoundaryChanges(xc: string): UnboundedZone {
  ...
  let fullRow = false;
  let hasHeader = false;
  const firstRangePart = ranges[0].trim();
  const secondRangePart = ranges[1] && ranges[1].trim();
 ...
 *
 */
export function toUnboundZone(xc: string): UnboundedZone {
 ...
 * Use toUnboundZone() instead for unbound zones.
 *
 * Examples:
 *    "A1" ==> Top 0, Bottom 0, Left: 0, Right: 0
 *    "B1:B3" ==> Top 0, Bottom 3, Left: 1, Right: 1
...
export function createAdaptedZone<Z extends UnboundedZone | Zone>(
  zone: Z,
