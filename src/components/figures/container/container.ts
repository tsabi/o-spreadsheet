...
export class FiguresContainer extends Component<Props, SpreadsheetChildEnv> {
  ...
  getStyle(info: FigureInfo) {
    // TODO should be adapted - the goal will be to make sure that a graph is ALWAYS displayed in the bottomright pane
    ...
    const { offsetX, offsetY } = this.env.model.getters.getViewportOffsetInfo();
    let { width, height } = target;
    let x = target.x - offsetX - 1;
    let y = target.y - offsetY - 1;

    if (x < offsetCorrectionX) {
      width -= offsetCorrectionX - x;
      x = offsetCorrectionX;
    }
    if (y < offsetCorrectionY) {
      height -= offsetCorrectionY - y;
      y = offsetCorrectionY;
    }
