...
import { extractIconColor, getFontSizeMatchingWidth } from "../../helpers";
...
const CHART_HORIZONTAL_PADDING_RATIO = 0.025;
...
const TEMPLATE = xml/* xml */ `
  <div class="o-scorecard" t-att-style="chartStyle">
    ...
    <t t-if="title">
      <div class="o-title-text" t-esc="title" t-att-style="textStyles.titleStyle"></div>
    </t>
    <div class="o-key-text" t-esc="keyValue" t-att-style="textStyles.keyStyle"></div>
    <t t-if="baseline || baselineDescr">
      <div class="o-baseline-text" t-att-style="textStyles.baselineStyle">
        <t t-if="baseline">
          ...
          <span t-att-style="baselineColor">
            <t t-esc="baseline" />
          </span>
        </t>
        <span t-att-style="textStyles.baselineDescrStyle"><t t-esc="baselineDescr" /></span>
      </div>
    </t>
  </div>
`;
...
export class ScorecardChart extends Component<Props, SpreadsheetChildEnv> {
  ...
  get baselineColor(): string {
    ...
    if (diff > 0) {
      style = `color:${extractIconColor(ARROW_UP)}`;
    } else if (diff < 0) {
      style = `color:${extractIconColor(ARROW_DOWN)}`;
    }
    return style;
  }
  ...
  private getWidestElement(): ScorecardElement {
    const titleWidth = this.getElementWidth("title", TITLE_BOX_HEIGHT_RATIO * 10);
    const keyValueWidth = this.getElementWidth("key", KEY_BOX_HEIGHT_RATIO * 10);

    let widest: ScorecardElement = titleWidth > keyValueWidth ? "title" : "key";

    const baselineWidth = this.getElementWidth("baseline", BASELINE_BOX_HEIGHT_RATIO * 10);
  ...
  private getElementWidth(text: ScorecardElement, fontSize: number) {
    ...
    switch (text) {
      case "baseline":
        ...
        const largeText =
          this.baselineArrowDirection !== "neutral" ? "0" + this.baseline : this.baseline;
