...
export class Session extends EventBus<CollaborativeEvent> {
  ...
  private uuidGenetator = new UuidGenerator();
  /**
  ...
  save(commands: CoreCommand[], changes: HistoryChange[]) {
    ...
    const revision = new Revision(this.uuidGenetator.uuidv4(), this.clientId, commands, changes);
  ...
  undo(revisionId: UID) {
    this.sendUpdateMessage({
      ...
      version: MESSAGE_VERSION,
      serverRevisionId: this.serverRevisionId,
      nextRevisionId: this.uuidGenetator.uuidv4(),
  ...
  snapshot(data: WorkbookData) {
    const snapshotId = this.uuidGenetator.uuidv4();
    this.transportService.sendMessage({
      type: "SNAPSHOT",
