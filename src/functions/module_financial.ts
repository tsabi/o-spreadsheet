...
export const DDB: AddFunctionDescription = {
  ...
  compute: function (
    ...
    let currentCost = _cost;
    let deprec = currentCost * deprecFactor;
    for (let i = 0; i < _period; i++) {
      deprec = currentCost * deprecFactor;
      if (currentCost - deprec < _salvage) {
        deprec = currentCost - _salvage;
        currentCost = _salvage;
      } else {
        currentCost -= deprec;
      }
    }
...
export const DOLLARDE: AddFunctionDescription = {
  ...
  args: args(`
      fractional_price (date) ${_lt(
        "The price quotation given using fractional decimal conventions."
      )}
      unit (date) ${_lt("The units of the fraction, e.g. 8 for 1/8ths or 32 for 1/32nds.")}
    `),
  returns: ["NUMBER"],
...
export const DOLLARFR: AddFunctionDescription = {
  ...
  args: args(`
  decimal_price (date) ${_lt("The price quotation given as a decimal value.")}
      unit (date) ${_lt("The units of the desired fraction, e.g. 8 for 1/8ths or 32 for 1/32nds.")}
    `),
  returns: ["NUMBER"],
...
export const PRICEMAT: AddFunctionDescription = {
  ...
  compute: function (
     ...
     *
     * I have not been able to figure out how exactly Excel handles these cases.
