  ...
  for (let sheet of convertedSheets) {
    ...
    for (let table of tables) {
      ...
      for (let position of positions(toZone(table.ref))) {
        const xc = toXC(position[0], position[1]);
        const cell = sheet.cells[xc];
 ...
 *
 * A reference in a table can have the form (only the part between brackets shouldd be given to this function):
  ...
  // Other references
    ...
    switch (refElements[0].slice(1, refElements[0].length - 1)) {
      ...
      case "#Data":
        ...
        break;
      case "#This Row":
        refZone.top = refZone.bottom = toCartesian(cellXc)[1];
        break;
      case "#Headers":
