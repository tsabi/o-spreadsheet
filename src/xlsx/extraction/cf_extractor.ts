...
export class XlsxCfExtractor extends XlsxBaseExtractor {
  ...
  public extractConditionalFormattings(): XLSXConditionalFormat[] {
    const cfs = this.mapOnElements(
      { document: this.rootFile.file, query: "worksheet > conditionalFormatting" },
    ...
    // XLSX exension to OpenXml
    cfs.push(
      ...this.mapOnElements(
        { document: this.rootFile.file, query: "extLst x14:conditionalFormatting" },
        (cfElement): XLSXConditionalFormat => {
          return {
    ...
    return this.mapOnElements(
      ...
      (cfRuleElement): XLSXCFRule => {
        ...
        if (cfType === "dataBar") {
          ...
          throw new Error(_lt("Databars conditional formats are not supported."));
        }
    ...
    return this.mapOnElements(
      ...
      (cfVoElement): XLSXCFValueObject => {
        return {
          ...
          gte: this.extractAttr(cfVoElement, "gte").asBool(),
