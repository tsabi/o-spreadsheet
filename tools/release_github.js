/**
 * Largely inspired by the release script of owl:
 * https://github.com/odoo/owl/blob/master/tools/release.js
 */
...
const branchName = () => "14.0";

const REL_NOTES_FILE = `release-notes.md`;
const STEPS = 8;
const branch = "master";

const BRANCH_REGEX = /^(\d{2}\.0)$|^(saas-\d{2}\.\d)$/;
...
async function startRelease() {
  // Check that we are on a base branch
  if (!branchName().match(BRANCH_REGEX)) {
    logError(`You shall not pass! You are not on a base branch, you are on '${branch}'!`);
    return;
  }
  ...
  const [major, minor, release] = package.version.split(".");
  const nextVersion = `${major}.${minor}.${parseInt(release, 10) + 1}`;
  ...
  //   draft = "--draft";
  // }
  // let shouldUploadPlayground = await ask(
  //   `Should this release be uploaded on the playground [y/n] ? (y)`
  // );
